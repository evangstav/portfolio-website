...
- All user-facing routes live under `src/app/[locale]/...` (`/`, `/biography`, `/media`). The root `src/app/layout.tsx` is intentionally a passthrough — the real `<html>`, `<body>`, and `NextIntlClientProvider` are in `src/app/[locale]/layout.tsx`.
